...
if {! [file exists $base_dir/mp_txrx_Scrambled_shit_subsystem]} {
    file delete -force $base_dir/mp_txrx_Scrambled_shit_subsystem
}

project new $base_dir mp_txrx_Scrambled_shit_subsystem
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/mp/mp_txrx_Scrambled_shit_subsystem_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/mp/mp_txrx_Scrambled_shit_subsystem_atb.vhd
vcom -quiet $base_dir/mp/mp_txrx_Scrambled_shit_subsystem_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_5_qv} mismatch] == 0 && $mismatch } {
                report_mismatch out_5_qv $cyclenum
            }
            if { [catch {exa mismatch_out_4_qc} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_qc $cyclenum
            }
            if { [catch {exa mismatch_out_1_Out1} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_Out1 $cyclenum
            }
            if { [catch {exa mismatch_out_2_Out2} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_Out2 $cyclenum
            }
            if { [catch {exa mismatch_out_3_pwr} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_pwr $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps mp_txrx_Scrambled_shit_subsystem_atb 
    do $base_dir/mp/mp_txrx_Scrambled_shit_subsystem_atb.wav.do
