...
if {! [file exists $base_dir/final_project_1_txrx_fp_Symbol_sample_adaptation]} {
    file delete -force $base_dir/final_project_1_txrx_fp_Symbol_sample_adaptation
}

project new $base_dir final_project_1_txrx_fp_Symbol_sample_adaptation
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/final_project_1/final_project_1_txrx_fp_Symbol_sample_adaptation_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/final_project_1/final_project_1_txrx_fp_Symbol_sample_adaptation_atb.vhd
vcom -quiet $base_dir/final_project_1/final_project_1_txrx_fp_Symbol_sample_adaptation_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_v_sample_update} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_v_sample_update $cyclenum
            }
            if { [catch {exa mismatch_out_2_qc} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_qc $cyclenum
            }
            if { [catch {exa mismatch_out_3_sample_crement} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_sample_crement $cyclenum
            }
            if { [catch {exa mismatch_out_4_sym_sample} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_sym_sample $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps final_project_1_txrx_fp_Symbol_sample_adaptation_atb 
    do $base_dir/final_project_1/final_project_1_txrx_fp_Symbol_sample_adaptation_atb.wav.do
