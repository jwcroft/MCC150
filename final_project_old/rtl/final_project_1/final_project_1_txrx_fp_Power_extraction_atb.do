...
if {! [file exists $base_dir/final_project_1_txrx_fp_Power_extraction]} {
    file delete -force $base_dir/final_project_1_txrx_fp_Power_extraction
}

project new $base_dir final_project_1_txrx_fp_Power_extraction
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/final_project_1/final_project_1_txrx_fp_Power_extraction_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/final_project_1/final_project_1_txrx_fp_Power_extraction_atb.vhd
vcom -quiet $base_dir/final_project_1/final_project_1_txrx_fp_Power_extraction_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_qv} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_qv $cyclenum
            }
            if { [catch {exa mismatch_out_2_qc} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_qc $cyclenum
            }
            if { [catch {exa mismatch_out_3_left_power} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_left_power $cyclenum
            }
            if { [catch {exa mismatch_out_4_main_power} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_main_power $cyclenum
            }
            if { [catch {exa mismatch_out_5_right_power} mismatch] == 0 && $mismatch } {
                report_mismatch out_5_right_power $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps final_project_1_txrx_fp_Power_extraction_atb 
    do $base_dir/final_project_1/final_project_1_txrx_fp_Power_extraction_atb.wav.do
