...
if {! [file exists $base_dir/mp_txrx_Scale]} {
    file delete -force $base_dir/mp_txrx_Scale
}

project new $base_dir mp_txrx_Scale
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/mp/mp_txrx_Scale_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/mp/mp_txrx_Scale_atb.vhd
vcom -quiet $base_dir/mp/mp_txrx_Scale_stm.vhd
...
if {$vcomfailed == 0} {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps mp_txrx_Scale_atb 
    do $base_dir/mp/mp_txrx_Scale_atb.wav.do
