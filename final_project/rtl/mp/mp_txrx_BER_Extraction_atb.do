...
if {! [file exists $base_dir/mp_txrx_BER_Extraction]} {
    file delete -force $base_dir/mp_txrx_BER_Extraction
}

project new $base_dir mp_txrx_BER_Extraction
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/mp/mp_txrx_BER_Extraction_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/mp/mp_txrx_BER_Extraction_atb.vhd
vcom -quiet $base_dir/mp/mp_txrx_BER_Extraction_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_BER} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_BER $cyclenum
            }
            if { [catch {exa mismatch_out_4_bit_error} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_bit_error $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps mp_txrx_BER_Extraction_atb 
    do $base_dir/mp/mp_txrx_BER_Extraction_atb.wav.do
