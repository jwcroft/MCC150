...
if {! [file exists $base_dir/mp_txrx_Memory]} {
    file delete -force $base_dir/mp_txrx_Memory
}

project new $base_dir mp_txrx_Memory
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/mp/mp_txrx_Memory_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/mp/mp_txrx_Memory_atb.vhd
vcom -quiet $base_dir/mp/mp_txrx_Memory_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_m2} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_m2 $cyclenum
            }
            ...
            if { [catch {exa mismatch_out_4_m1} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_m1 $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps mp_txrx_Memory_atb 
    do $base_dir/mp/mp_txrx_Memory_atb.wav.do
