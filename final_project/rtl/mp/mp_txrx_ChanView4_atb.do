...
if {! [file exists $base_dir/mp_txrx_ChanView4]} {
    file delete -force $base_dir/mp_txrx_ChanView4
}

project new $base_dir mp_txrx_ChanView4
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/mp/mp_txrx_ChanView4_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/mp/mp_txrx_ChanView4_atb.vhd
vcom -quiet $base_dir/mp/mp_txrx_ChanView4_stm.vhd
...
if {$vcomfailed == 0} {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps mp_txrx_ChanView4_atb 
    do $base_dir/mp/mp_txrx_ChanView4_atb.wav.do
