...
if {! [file exists $base_dir/mp_txrx_Symbol_Recovery]} {
    file delete -force $base_dir/mp_txrx_Symbol_Recovery
}

project new $base_dir mp_txrx_Symbol_Recovery
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/mp/mp_txrx_Symbol_Recovery_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/mp/mp_txrx_Symbol_Recovery_atb.vhd
vcom -quiet $base_dir/mp/mp_txrx_Symbol_Recovery_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_bits} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_bits $cyclenum
            }
            if { [catch {exa mismatch_out_4_phase} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_phase $cyclenum
            }
            if { [catch {exa mismatch_out_5_trigger} mismatch] == 0 && $mismatch } {
                report_mismatch out_5_trigger $cyclenum
            }
            if { [catch {exa mismatch_out_6_sym_pwr} mismatch] == 0 && $mismatch } {
                report_mismatch out_6_sym_pwr $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps mp_txrx_Symbol_Recovery_atb 
    do $base_dir/mp/mp_txrx_Symbol_Recovery_atb.wav.do
