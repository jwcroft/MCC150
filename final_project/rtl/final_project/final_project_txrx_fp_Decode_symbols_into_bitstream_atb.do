...
if {! [file exists $base_dir/final_project_txrx_fp_Decode_symbols_into_bitstream]} {
    file delete -force $base_dir/final_project_txrx_fp_Decode_symbols_into_bitstream
}

project new $base_dir final_project_txrx_fp_Decode_symbols_into_bitstream
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/final_project/final_project_txrx_fp_Decode_symbols_into_bitstream_fpc.do"
} else {
...
vcom -quiet -O0 $base_dir/final_project/final_project_txrx_fp_Decode_symbols_into_bitstream_atb.vhd
vcom -quiet $base_dir/final_project/final_project_txrx_fp_Decode_symbols_into_bitstream_stm.vhd
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_v_bits} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_v_bits $cyclenum
            }
            if { [catch {exa mismatch_out_2_qc} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_qc $cyclenum
            }
            if { [catch {exa mismatch_out_3_bits} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_bits $cyclenum
            }
            if { [catch {exa mismatch_out_4_out4} mismatch] == 0 && $mismatch } {
                report_mismatch out_4_out4 $cyclenum
            }
            if { [catch {exa mismatch_out_5_out5} mismatch] == 0 && $mismatch } {
                report_mismatch out_5_out5 $cyclenum
            }
        } else {
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps final_project_txrx_fp_Decode_symbols_into_bitstream_atb 
    do $base_dir/final_project/final_project_txrx_fp_Decode_symbols_into_bitstream_atb.wav.do
