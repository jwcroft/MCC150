...
if {! [file exists $base_dir/final_project_txrx_fp_Phase_extraction_and_synchronization]} {
    file delete -force $base_dir/final_project_txrx_fp_Phase_extraction_and_synchronization
}

project new $base_dir final_project_txrx_fp_Phase_extraction_and_synchronization
...
if {[info exists quartus_dir] && [file isdirectory $quartus_dir]} {
    ...
    do "$base_dir/final_project/final_project_txrx_fp_Phase_extraction_and_synchronization_fpc.do"
} else {
...
if {$vcomfailed == 0} {
    onbreak {
        ...
        if {$regOK == 1} {
            ...
            if { [catch {exa mismatch_out_1_v_sym_update} mismatch] == 0 && $mismatch } {
                report_mismatch out_1_v_sym_update $cyclenum
            }
            if { [catch {exa mismatch_out_2_qc} mismatch] == 0 && $mismatch } {
                report_mismatch out_2_qc $cyclenum
            }
            if { [catch {exa mismatch_out_3_symbols_phase_x} mismatch] == 0 && $mismatch } {
                report_mismatch out_3_symbols_phase_x $cyclenum
            }
    ...
    vsim -quiet -error 3473 -msgmode both -voptargs="+acc" -t ps final_project_txrx_fp_Phase_extraction_and_synchronization_atb 
    do $base_dir/final_project/final_project_txrx_fp_Phase_extraction_and_synchronization_atb.wav.do
